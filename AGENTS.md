...
- For `herdr-theme/` (Bun + OpenTUI/Solid TUI behind the global `herdr-theme` and `herdrx` commands), exclude: `node_modules/`, `dist/`, `backups/`, `watch.log`. Its own `config.json` (light/dark theme pair) is tracked.
